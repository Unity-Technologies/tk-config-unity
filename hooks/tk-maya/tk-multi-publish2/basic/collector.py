...
class MayaSessionCollectorExt(HookBaseClass):
    ...
    def settings(self):
        ...
        maya_publish_settings = {
            "FBX Publish Template": {
                "type": "template",
                "default": None,
                "description": "Template path for FBX files published by Maya. Must "
            ...
            "FBX Work Template": {
                "type": "template",
                "default": None,
                "description": "Template path for FBX work files, in Maya. Must "
                               "correspond to a template defined in "
                               "templates.yml.",
            }
        }
    ...
    def process_current_session(self, settings, parent_item):
        ...
        self.logger.info("Augmenting Maya session")
        if parent_item.children:
            allChildren = list(parent_item.children)
            if allChildren:                    
                self.collect_fbx_files(allChildren[0], allChildren[0].properties["project_root"], settings)
    ...
    def collect_fbx_files(self, parent_item, project_root, settings):
        ...
        publish_template_setting = settings.get("FBX Publish Template")
        publish_template = self.parent.engine.get_template_by_name(
            publish_template_setting.value)
            
        work_template_setting = settings.get("FBX Work Template")
        ...
        if work_template:
            ...
            if publish_template:
                ...
                for file in os.listdir(publish_path):
                    if not file.startswith(fields['name']) or not file.endswith(".fbx"):
                        continue
        ...
        item.name = os.path.basename(fbx_path)
